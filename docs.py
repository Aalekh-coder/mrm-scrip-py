...
if __name__ == "__main__":
    ...
    domain_label = report["domain"].replace(".", "_")
    output_file = f"doc_scraper_{domain_label}.json"
    ...
    print(f"\n{'='*60}")
    print(f"  Done.")
    print(f"  Documents processed : {report['total_processed']}")
    print(f"  Documents skipped   : {report['total_skipped']}")
    print(f"  Emails leaked       : {len(report['summary']['unique_emails_leaked'])}")
    print(f"  Authors found       : {len(report['summary']['authors_found'])}")
    print(f"  Software versions   : {len(report['summary']['software_versions_leaked'])}")
